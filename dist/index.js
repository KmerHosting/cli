...
function confirmDestructive(action, id, ctx) {
    if (ctx.yes)
        return;
...
function printHuman(value) {
    if (Array.isArray(value)) {
        ...
        if (value.every((item) => item && typeof item === "object" && !Array.isArray(item))) {
            ...
            for (const row of rows)
                console.log(keys.map((key, index) => formatValue(row[key]).slice(0, 32).padEnd(widths[index])).join("  "));
            return;
        }
    ...
    if (value && typeof value === "object") {
        for (const [key, entry] of Object.entries(value))
            console.log(`${key}: ${formatValue(entry)}`);
        return;
    }
    console.log(formatValue(value));
}
function output(result, ctx) {
    if (ctx.json)
        console.log(JSON.stringify(result, null, 2));
    else
        printHuman(dataOf(result));
}
async function run(args) {
    ...
    let result;
    if (resource === "account" && command === "get")
        result = await client.account.get();
    else if (resource === "account" && command === "api-usage")
        result = await client.account.apiUsage();
    else if (resource === "services" && command === "list")
        result = await client.services.list();
    else if (resource === "services" && (command === "view" || command === "get"))
        result = await client.services.get(required(rest[0], "service id"));
    else if (resource === "domains" && command === "list")
        result = await client.domains.list();
    else if (resource === "domains" && (command === "view" || command === "get"))
        result = await client.domains.get(required(rest[0], "domain id"));
    else if (resource === "domains" && command === "dns")
        result = await runDomainDns(client, rest, ctx);
    else if (resource === "domains" && command === "auto-renew") {
        const id = required(rest[0], "domain id");
        const state = required(rest[1], "on or off");
        if (state !== "on" && state !== "off")
            throw new Error("Domain auto-renew must be on or off.");
    ...
    else if (resource === "email" && command === "services")
        result = await client.email.listServices();
    else if (resource === "email" && command === "provision")
        result = await client.email.provision(required(rest[0], "email service id"), ctx.mutationOptions);
    else if (resource === "email" && command === "dns-sync")
        result = await client.email.syncDns(required(rest[0], "email service id"), ctx.mutationOptions);
    else if (resource === "hosting" && command === "services")
        result = await client.hosting.listServices();
    else if (resource === "hosting" && command === "stats")
        result = await client.hosting.stats(required(rest[0], "hosting service id"));
    else if (resource === "hosting" && command === "panel-access") {
        const target = flag(args, "target");
        if (target && target !== "panel" && target !== "filemanager")
            throw new Error("--target must be panel or filemanager.");
        result = await client.hosting.createPanelAccess(required(rest[0], "hosting service id"), target, ctx.mutationOptions);
    }
    else if (resource === "lxc" && command === "list")
        result = await client.lxc.list();
    else if (resource === "lxc" && (command === "view" || command === "get"))
        result = await client.lxc.get(required(rest[0], "LXC instance id"));
    else if (resource === "lxc" && command === "metrics")
        result = await client.lxc.metrics(required(rest[0], "LXC instance id"));
    else if (resource === "lxc" && command === "action") {
        const action = required(rest[1], "LXC action");
        if (!["start", "stop", "restart", "freeze"].includes(action))
            throw new Error("LXC action must be start, stop, restart, or freeze.");
        if (DESTRUCTIVE_ACTIONS.has(action))
            confirmDestructive(action, required(rest[0], "LXC instance id"), ctx);
        ...
        if (snapshotCommand === "list")
            result = await client.lxc.snapshots.list(id);
        else {
            const name = required(rest[2], "snapshot name");
            if (snapshotCommand === "delete" || snapshotCommand === "restore")
                confirmDestructive(`snapshot ${snapshotCommand}`, name, ctx);
            if (!["create", "delete", "restore"].includes(snapshotCommand))
                throw new Error("Snapshot command must be list, create, delete, or restore.");
            result = await client.lxc.snapshots.mutate(id, snapshotCommand, name, ctx.mutationOptions);
        }
    }
        ...
        const state = required(rest[1], "on or off");
        if (state !== "on" && state !== "off")
            throw new Error("LXC auto-renew must be on or off.");
        if (state === "off")
            confirmDestructive("disable LXC auto-renew", id, ctx);
        ...
        if (![1, 3, 6, 12].includes(billingMonths))
            throw new Error("LXC billing period must be 1, 3, 6, or 12 months.");
        result = await client.lxc.setBillingPeriod(id, billingMonths, ctx.mutationOptions);
    }
    else if (resource === "kvm" && command === "list")
        result = await client.kvm.list();
    else if (resource === "kvm" && (command === "view" || command === "get"))
        result = await client.kvm.get(required(rest[0], "KVM instance id"));
    else if (resource === "kvm" && command === "action") {
        const id = required(rest[0], "KVM instance id");
        const action = required(rest[1], "KVM action");
        if (!["start", "stop", "shutdown", "restart"].includes(action))
            throw new Error("KVM action must be start, stop, shutdown, or restart.");
        if (DESTRUCTIVE_ACTIONS.has(action))
            confirmDestructive(action, id, ctx);
        ...
        const state = required(rest[1], "on or off");
        if (state !== "on" && state !== "off")
            throw new Error("KVM auto-renew must be on or off.");
        ...
        const id = required(rest[0], "KVM instance id");
        const kvm = client.kvm;
        if (command === "renew") {
            if (!kvm.renew)
                throw new Error("Install the latest KmerHosting SDK before using KVM renewal.");
            const months = rest[1] ? Number(rest[1]) : undefined;
            result = await kvm.renew(id, months, ctx.mutationOptions);
        }
        else if (command === "cancel") {
            confirmDestructive("cancel KVM service", id, ctx);
            if (!kvm.cancel)
                throw new Error("Install the latest KmerHosting SDK before using KVM cancellation.");
            result = await kvm.cancel(id, ctx.mutationOptions);
        }
        else if (command === "keep-service") {
            if (!kvm.keepService)
                throw new Error("Install the latest KmerHosting SDK before using KVM recovery.");
            result = await kvm.keepService(id, ctx.mutationOptions);
        }
        else {
            confirmDestructive("reset KVM root password", id, ctx);
            if (!kvm.resetPassword)
                throw new Error("Install the latest KmerHosting SDK before using KVM password reset.");
            result = await kvm.resetPassword(id, required(rest[1], "new root password"), ctx.mutationOptions);
        }
    }
    else if (resource === "kvm" && command === "snapshots")
        result = await runKvmSnapshots(client, rest, ctx);
    else if (resource === "vps")
        throw new Error("The `vps` resource was retired. Use `lxc` for LXC inventory or `kvm` for KVM management.");
    else
        throw new Error("Unknown command. Run `kmerhosting --help` for usage.");
    output(result, ctx);
}
async function runDomainDns(client, args, ctx) {
    const [command, domainId, extra] = args;
    const id = required(domainId, "domain id");
    if (command === "list")
        return client.domains.dns.list(id);
    if (command === "create")
        return client.domains.dns.create(id, parseJson(required(extra, "DNS record JSON"), "DNS record"), ctx.mutationOptions);
    if (command === "update") {
...
async function runKvmSnapshots(client, args, ctx) {
    const [command, serviceId, extra] = args;
    const id = required(serviceId, "KVM instance id");
    if (command === "list")
        return client.kvm.snapshots.list(id);
    if (command === "create")
        return client.kvm.snapshots.create(id, { name: required(extra, "snapshot name"), description: args[3] }, ctx.mutationOptions);
    if (command === "update")
        return client.kvm.snapshots.update(id, required(extra, "snapshot id"), { name: args[3], description: args[4] }, ctx.mutationOptions);
    if (command === "delete") {
